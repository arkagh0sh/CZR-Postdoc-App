...
sets constructed from a data domain, that are finite up to symmetries (automorphisms) of the data domain.
...
\emphdef{polynomial rings with data values as variables},
...
invariant under the automorphisms of the underlying data domain.
...
%
To highlight my contributions to the state of the art,
citations to articles for which I am a co-author (for eg, \mine{GL25}) are coloured in red.
...
%\emphdef{polynomial rings with data values as variables},
%and \emphdef{orbit-finitely generated monoids}.
%A sub-object (vector subspaces, polynomial ideals, sub-monoids) of such an algebraic object is called \emphdef{equivariant} if it is invariant under the automorphisms of the underlying data domain.
...
The articles \cite{HR22}, \mine{GHL22} and \mine{GHL25} studies orbit-finite systems of linear equations and inequalities,
and as a special case show that the membership problems for finitely generated equivariant vector subspaces and cones are decidable.
As a corollary we get decidability of solvability of state equations of data Petri nets each of whose tokens carry a tuple of bounded length of
elements from the equality domain.
Reachability problem for such data Petri nets is undecidable
(\cite{Las16}).
Therefore solvability of state equations gives us a decidable over-approximation for reachability.
%
\paragraph*{Polynomial rings with data values as variables.}
%
The polynomial ring with elements from the equality data domains as variables was studied in \cite{Cohen67},
...
We extended this result in \mine{GL24} for polynomial rings with variables being elements coming from any data domain.
We showed that a polynomial ring such as this is Noetherian exactly when the finite induced substructures labelled with natural numbers are well-quasi-ordered under embeddability.
...
In \mine{GL25} we extended the results of \mine{GL24} by showing existence and computability of finite Gr\"{o}bner bases for the class of data domains that satisfy the well-quasi-ordering condition.
...
Both of these data domains satisfy our well-quasi-ordering condition.


The results of \mine{GL25} also implies decidability of the membership problem of finitely generated vector subspaces of orbit-finite dimensional vector spaces over these data domains.
...
%
%\newpage
\subsection*{Equivariant vector subspaces}\label{subsec:equiv}
%
...
%
\subsection*{Orbit-finite spanning problems}\label{subsec:span}
%
...
%\newpage
%
\subsection{Polynomial rings with data values as variables}\label{sec:poly}
%
In this setting, we study polynomial rings $\poly{\Q}{\M}$ with rational coefficients and elements from a data domain $\M$ as variables.
Automorphisms of $\M$ act on polynomials in a variable-wise manner.
For example,
for an automorphism $\pi$ of $\M$ and $x,y,z\in\M$,
\[
\pi(x - 3y^2z + 4) = \pi(x) - 3\pi(y)^2\pi(z) + 4
\]
...
\end{enumerate}
%
In \mine{GL24} we showed that every equivariant ideal in $\poly{\Q}{\M}$ has a orbit-finite set of generators if and only if finite substructures of $\M$ labelled with natural numbers, are well-quasi-ordered under embeddability.
...
every orbit-finite set of polynomials $G$ in $\poly{\Q}{\M}$ that has a common root,
...
%\arka{be more precise}
%
%\newpage
\subsection{Orbit-finitely generated monoids}\label{sec:ofmon}
%
%\arka{add example?}
The article \cite{dataMon} conjectured that the Sch\"{u}tzenberger-McNaughton-Papert theorem extends to register automata:
\begin{conjecture}
...
which is a strictly weaker model \cite{dataMon}.
...
%
The research on the three kinds of algebraic objects (orbit-finite dimensional vector spaces, polynomial rings with data values as variables, and orbit-finitely generated monoids) can be carried out independently.
...
%
We intend to have one summer intern every year.
The interns will either have
\begin{enumerate}
\item basic knowledge on model theory and abstract algebra,
in which case they will work on the core problems of the project, or
\item experience in programming, in which case they will work in implementation of the resulting algorithms.
\end{enumerate} 

\arka{Are there master's students in CAS? Can we work with masters students outside CAS?}
%
...
(\cite{BKM21},\mine{GL24},\cite{dataMon}).
...
his expertise on model theory will be very useful.
%
...
Both of them have made fundamental contributions to the study of linear noetherianity (\Cref{def:lin noe,conj:lin noeth}).

Finally, we will also collaborate with Miko\l{}aj Boja\'{n}czyk, S\l{}awomir Lasota and Piotr Hofman in \textbf{University of Warsaw}.
...
%
We intend to make one research visit per year to each of the point of collaborations mentioned above.
This leads to three visits every year.
In addition to collaboration on the core problems of the project,
we will also present our results in research seminars during these visits.

We expect two publications in peer-reviewed conference proceedings per year on average.
Therefore, apart from the research visits, we expect two conference travels per year on average.
%
